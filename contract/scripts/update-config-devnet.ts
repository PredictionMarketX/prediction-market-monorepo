/**
 * Update Prediction Market Config on Devnet
 * 更新 min_usdc_liquidity 为 10 USDC
 */
...
async function main() {
  ...
  try {
    ...
    console.log('Team Wallet:', currentConfig.teamWallet.toString());
    console.log('Min USDC Liquidity:', currentConfig.minUsdcLiquidity.toString(), '(当前)');
  } catch (err) {
    console.error('\n❌ 无法获取配置，请先初始化');
    throw err;
  }

  // 创建新配置对象 - 保持其他字段不变，只修改 minUsdcLiquidity
  const newConfig = {
    ...
    minTradingLiquidity: currentConfig.minTradingLiquidity,
    ...
    minUsdcLiquidity: new BN(10_000_000),  // ✅ 更新为 10 USDC
  ...
  try {
    ...
    console.log('Team Wallet:', updatedConfig.teamWallet.toString());
